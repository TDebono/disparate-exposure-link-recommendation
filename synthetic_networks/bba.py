...
def DPA(N, fm, d, plo_M, plo_m, verbose=False, seed=None):
    ...
    if verbose:
        ...
        print("Activity Power-Law outdegree: M={}, F={}".format(plo_M, plo_m))
        print('')
    ...
    while G.number_of_edges() < E:
        ...
        if verbose:
            ls = labels[source]
            lt = labels[target]
            print("{}->{} ({}{}): {}".format(ns, nt, 'F' if ls else 'M', 'F' if lt else 'M', G.number_of_edges()))
    ...
    if verbose:
        ...
        for k in [0,1]:
            ...
            print("{}: alpha={}, sigma={}, min={}, max={}".format('F' if k else 'M',
                                                                  fit.power_law.alpha, 
                                                                  fit.power_law.sigma, 
                                                                  fit.power_law.xmin, 
                                                                  fit.power_law.xmax))
